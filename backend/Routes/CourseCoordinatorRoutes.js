...
// To check that logged academic member is a course coordinator.
router.get('/isCourseCoordinator', authenticateToken, async (req, res) => {
    ...
    if (academicMember.isCourseCoordinator) {
        const courses = await CourseModel.find({ course_coordinator: academicMember._id });
        const coursesNames = [];
        Array.prototype.map.call(courses, course => {
            coursesNames.push(course.name);
        });
        return res.status(200).json({ isCourseCoordinator: true, courses: coursesNames });
    }
    else return res.status(200).json({ isCourseCoordinator: false });
});
...
router
    .post('/courseSlots', authenticateToken, async (req, res) => {
        ...
        if (CCAcademicModel.isCourseCoordinator) {
