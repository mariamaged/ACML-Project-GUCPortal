...
router.put("/enterNewPass", authenticateToken, async (req, res) => {
    ...
    if (passNew != passCheck) {
        ...
        try {
            await StaffMemberModel.findByIdAndUpdate(req.user.id, {
...
router.put("/updateProfile", authenticateToken, async (req, res) => {
    ...
    //update email
    if (req.body.email) {
        const mailPresent = await StaffMemberModel.findOne({
    ...
    if (role == "HR") {
        ...
        const staff1 = await StaffMemberModel.findByIdAndUpdate(req.user.id, {
            email: email,
            office: offID,
        });
...
function compare(a, b) {
    ...
    if (
        moment(b.date).format("YYYY-MM-DD") < moment(a.date).format("YYYY-MM-DD")
    ) {
...
router.get("/missingDays", authenticateToken, async (req, res) => {
    ...
    for (var l = 0; l < sortedUserDays.length; l++) {
        console.log(
            " sortedUserDays" + moment(sortedUserDays[l].date).format("YYYY-MM-DD")
        );
    }
    var j = 0;
    var currDay = 11;
    ...
    if (j > 0) {
        ...
        var lastD = new moment(dateYear + "-" + dateMonth + "-" + currDay).format(
            "D"
        );
        var lastDName = new moment(
            dateYear + "-" + dateMonth + "-" + currDay
        ).format("dddd");
        const lastMonth = new moment(
            dateYear + "-" + dateMonth + "-" + currDay
        ).format("M");
        var lastYear = new moment(
            dateYear + "-" + dateMonth + "-" + currDay
        ).format("Y");
        console.log("ld= " + lastD + " lm= " + lastMonth + " ly= " + lastYear);
    ...
    if (
        ...
        while (lastDay < 32) {
            ...
            lastDay++;
            lastDName = new moment(lastYear + "-" + dateMonth + "-" + lastDay).format(
                "dddd"
            );
    ...
    if (
        sortedUserDays.length == 0 ||
        moment(sortedUserDays[sortedUserDays.length - 1].date).format("M") ==
        dateMonth
    ) {
        var currDay2 = 1;
        console.log("HEREEEEEEEEEEEEEEEEEEEEEEEEEEEE");
        lastDName = new moment(nextYear + "-" + nextMonth + "-" + currDay2).format(
            "dddd"
        );
        for (var g = 1; g <= 10; g++) {
            //added recently
            var currDate = new moment(
                nextYear + "-" + nextMonth + "-" + currDay2
            ).format("dddd");
            if (lastDName != "Friday" && lastDName != day_off) {
                missedDays[k++] = new moment(
                    nextYear + "-" + nextMonth + "-" + currDay2
    ...
    //else will need to check and only insert absent days
        ...
        var last2D = moment(sortedUserDays[sortedUserDays.length - 1].date).format(
            "D"
        );
        var last2day = parseInt(last2D) + 1;
    ...
    for (var d = 0; d < missedDays.length; d++) {
        if (
            moment(missedDays[d]).format("YYYY-MM-DD") <
            new moment().format("YYYY-MM-DD")
        )
        ...
        for (var i = 0; i < monthly.length; i++) {
            if (monthly[i].num == month && monthly[i].yearNum == year) {
                ...
                return res.send({
                    ...
                    extraMinutes: monthly[i].extraMinutes,
                });
            }
        }
    } else {
...
function calculateHrs(dateMonth, day_off) {
    ...
    if (dateMonth == 2) {
        if (moment(dateYear).isLeapYear()) end = 29;
        else end = 28;
    }
    const currYear = new moment().format("Y");
    for (var i = 11; i <= end; i++) {
        var currDay = new moment(currYear + "-" + dateMonth + "-" + i).format(
            "dddd"
        );
    ...
    for (var j = 1; j <= 10; j++) {
        var currDay = new moment(currYear + "-" + dateMonth + "-" + j).format(
            "dddd"
        );
        if (currDay != day_off && currDay != "Friday") {
            hours += 8;
